...
class Settings(BaseSettings):
    ...
    api_secret: str = Field(
        validation_alias=AliasChoices("alpaca_api_secret", "apca_api_secret_key")
    )
    data_feed: str = Field(
        default="iex",
        validation_alias=AliasChoices("alpaca_data_feed", "apca_data_feed"),
    )
    base_url: str = Field(
        default="https://data.alpaca.markets/v2",
        validation_alias=AliasChoices(
            "alpaca_base_url", "apca_api_base_url", "apca_api_data_url"
        ),
    )
    trading_base_url: str = Field(
        default="https://paper-api.alpaca.markets",
        validation_alias=AliasChoices(
            "alpaca_trading_base_url", "apca_api_trading_url"
        ),
    )
