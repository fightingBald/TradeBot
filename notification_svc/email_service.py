    ...
    def send_email(
        ...
        *,
        ...
        Args:
            ...
            recipients: Primary recipients (To).
            subtype: MIME subtype for the body, e.g. ``plain`` or ``html``.
            cc: Optional CC recipients.
            bcc: Optional BCC recipients (not visible to other recipients).
            attachments: Optional list of attachments.
            headers: Optional additional MIME headers.
            reply_to: Overrides default reply-to address.
        ...
        message["To"] = ", ".join(str(addr) for addr in recipients)
        if cc:
            message["Cc"] = ", ".join(str(addr) for addr in cc)
        resolved_reply_to = reply_to or self._settings.reply_to
        ...
        all_recipients: MutableSequence[str] = [str(addr) for addr in recipients]
        if cc:
            all_recipients.extend(str(addr) for addr in cc)
        if bcc:
            all_recipients.extend(str(addr) for addr in bcc)
