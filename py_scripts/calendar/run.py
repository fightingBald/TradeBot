...
from toolkits.calendar_svc.settings import build_runtime_options, load_config, load_env_file
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--google-create-calendar", action="store_true", help="Create Google Calendar when name not found"
    )
    ...
    parser.add_argument(
        "--macro-event-keywords", help="Comma separated macro event keywords to filter, e.g. FOMC,CPI,NFP"
    )
    parser.add_argument(
...
def main(args: Sequence[str] | None = None) -> None:
    ...
    try:
        options = build_runtime_options(parsed, config_data, config_base=config_base, project_root=project_root)
