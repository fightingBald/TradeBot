...
def test_diff_identifies_buys_sells_new_and_exits():
    old_snapshot = load_snapshot_csv(
        DATA_DIR / "ark_holdings_old" / "ARKW_2025-10-31.csv"
    )
    new_snapshot = load_snapshot_csv(
        DATA_DIR / "ark_holdings_new" / "ARKW_2025-10-31.csv"
    )
...
def test_diff_handles_spatial_etf_changes():
    old_snapshot = load_snapshot_csv(
        DATA_DIR / "ark_holdings_old" / "ARKX_2025-10-31.csv"
    )
    new_snapshot = load_snapshot_csv(
        DATA_DIR / "ark_holdings_new" / "ARKX_2025-10-31.csv"
    )
