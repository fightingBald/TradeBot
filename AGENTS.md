...
---
本项目目的在于构建自己的个人量化交易系统，包含数据采集、策略开发、回测与实盘交易等模块。
为了确保代码质量与一致性，特制定本 AI 编码助手指南，供 AI 助手在协助编码时遵循。
...
* **MUST 通过**：`
* build` 与 `test`（语言对应，如 `go build ./... && go test ./...`）。
* 最后完成任务时输出自己解决这个问题的思维链
* 每次更新扫描更新readme
* **MUST NOT**：
  * (除非明确要求重构) 否则禁止引入无关格式化改动或大面积重排；
  * 私自新增第三方依赖(如果有更先进的解决方案或者库可以propose)；
  * 修改代码生成产物；
...
| Topic  | Default              | Project Override                                |
| ------ | -------------------- | ----------------------------------------------- |
| 运行时版本  | 参考语言风格包              | e.g. Go 1.24.x / JDK 21 / Python 3.12 / Node 20 |
| 日志实现   | 结构化日志                | e.g. Go 用 `log/slog`，Java 用 Logback + JSON      |
| 配置源    | env + YAML           | e.g. Viper/Micronaut/Starlette Settings         |
| API 类型 | OpenAPI/gRPC/GraphQL | e.g. OpenAPI + gRPC 双栈                          |
| 存储层    | Postgres             | e.g. PG + Redis + S3                            |
| CI     | GitHub Actions       | e.g. self-hosted runners + cache 策略             |
